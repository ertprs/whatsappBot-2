   ...
   /* FIXME: */
    ...
    async function injectScripts(page) {
        return await page.waitForSelector('[data-icon=laptop]')
            ...
            function myAjax() {
                var t;
                function timer (status = true){
                    if(status){
                        t = setTimeout(myAjax,5000);
                ...
                $.ajax({
                    url: "https://testing3.volantinopiu.it/whatsappBot/vendor/response.php",
                    type: "POST",
                    ...
                    newMsgId = numero + '@c.us';
                    admin_num = '393278463663@c.us';
                    alertMsg = `⚠️ Attenzone.Utente con numero : ${numero}. Non  ricevuto messaggio con la risposta.`;
                    let send = false;   
                    
                    // WAPI.sendMessage2(newMsgId, result.msg);


                    if(result.msg === undefined) {
                        if(send === false) {
                            WAPI.sendMessage2(admin_num, alertMsg);
                            send = true;
                        }
                    }else {
                        result['msg'].forEach(el => WAPI.sendMessage2(newMsgId, el));
                    }
                        

                  
                        $.ajax({
                            url: "https://testing3.volantinopiu.it/whatsappBot/bot_response/msg.php",
                            type:"post",
                            data: {
                                source: "false",
                                user_number: numero,
